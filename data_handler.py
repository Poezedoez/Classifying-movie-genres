...
class DataHandler:
  ...
  def load_batch(self, data_type="train"):
    ...
    if (data_type == "val"):
      sampled = self.validation_data.sample(self.batch_size)
    ...
    targets = torch.FloatTensor(sampled.iloc[:, 2:].values)

    return torch.nn.utils.rnn.pad_sequence(features, batch_first=True), targets
